...
$background_image = get_field('image_highlightblock');
$tagline = get_field('tagline_highlightblock');
$title = get_field('title_highlightblock');
$description = get_field('description_highlightblock');

$have_location = get_field('have_location_blockhighlight');
$location = get_field('location_blockhighlight');


$button_type = get_field('button_type');
$button_link = get_field('button_link_highlightblock');
$button_download = get_field('button_download_highlightblock');
?>

<section class="relative text-white container-large ">
    <div
        class="flex flex-col relative p-8 md:p-10 justify-between min-h-[800px] rounded-3xl overflow-hidden wow animate__animated animate__fadeIn">
        ...
        <div class="z-10 flex items-center gap-3">
            ...
            <div class="flex flex-col">
                <?php if (!empty($location["title"])): ?>
                    <p class="text-16 text-beje-02"><?= esc_html($location["title"]) ?></p>
                <?php endif; ?>
                <?php if (!empty($location["description"])): ?>
                    <p class="text-grey-02 text-12px"><?= esc_html($location["description"]) ?></p>
                <?php endif; ?>
            </div>
        </div>
