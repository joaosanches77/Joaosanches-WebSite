...
$number_of_blocks = get_field('number_blocks');
$block_1 = get_field('block_1');
$block_2_items = get_field('block_2');
...
<div class="main-container">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 wow animate__animated animate__fadeIn">
        <?php
        ...
        <?php
        // CONDIÇÃO 2: Verificar se o Bloco 2 (Repetidor) tem itens
        if ($block_2_items):
            // Verificamos se tem mais de 1 item para decidir se é um carrossel
            $is_carousel = count($block_2_items) > 1;
            ?>
            <div
                class="col-span-12 lg:col-span-7 3xl:col-span-5 relative rounded-3xl overflow-hidden text-white h-full flex">

                <div class="<?php if ($is_carousel) {
                    echo 'owl-carousel block-2-carousel owl-theme';
                } ?> relative h-full flex-grow">

                    <?php foreach ($block_2_items as $item): // Loop através de cada item do repetidor ?>
                ...
                <?php if ($is_carousel): ?>
                    <div class="block-2-carousel-dots absolute top-10 right-10 flex flex-col space-y-2 z-10"></div>
                <?php endif; ?>
